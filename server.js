    ...
    //socket.emit("pick",mangtoados);
     ...
     for(var i=0; i < mangtoados.length; i+=2){
       if(mangtoados[i]==(data.x-480) && mangtoados[i+1]==data.y){
         ...
         // console.log(idConect);
         mangdelete.push(data.x,data.y);
         socket.to(socket.Phong).emit("pick-delete",mangdelete);
         socket.emit("pick",mangtoados);
         tonTai = 1;
         break;
       }
     ...
     if( tonTai == 0 ){
       mangcheck.push(data.x,data.y);
       console.log("check "+mangcheck);
       socket.to(socket.Phong).emit("pick-check",mangcheck);
     }
